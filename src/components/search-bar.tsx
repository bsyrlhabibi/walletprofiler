        ...
        <div className="relative flex items-center glass-card rounded-2xl shadow-lg shadow-fuchsia-200/30 dark:shadow-none">
          ...
          {showChainSelector && (
            <div ref={dropdownRef} className="relative ml-2 flex-shrink-0">
              ...
              {showChains && (
                <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-1 z-[200] min-w-[160px] animate-fade-in">
                  {CHAINS.map((c) => (
                    <button
                      key={c.id}
                      type="button"
          ...
          {/* Search icon */}
          <div className={`${showChainSelector ? "ml-2" : "ml-4"} p-2 rounded-xl bg-gray-50 dark:bg-fuchsia-950 flex-shrink-0`}>
            <Search className="w-5 h-5 text-gray-400 dark:text-fuchsia-500" />
          </div>
